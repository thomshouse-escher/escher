...
abstract class Helper_router extends Helper {
    ...
    function getCurrentPath($absolute=TRUE,$args=FALSE,$qsa=FALSE) {
        if ($dispatch = $this->getDispatchRouter()) {
            return $dispatch->getCurrentPath($absolute,$args,$qsa);
        }
        if ($absolute) {
            $CFG = Load::CFG();
            $root = $CFG['wwwroot'];
    ...
    function getParentPath($absolute=TRUE) {
        if ($dispatch = $this->getDispatchRouter()) {
            return $dispatch->getParentPath($absolute);
        }
        if ($absolute) {
            $CFG = Load::CFG();
            $root = $CFG['wwwroot'];
    ...
    function resolvePath($url,$absolute=TRUE) {
        if ($dispatch = $this->getDispatchRouter()) {
            return $dispatch->resolvePath($url,$absolute);
        }
        if (preg_match('/^#/',$url)) {
            return $url;
    ...
    protected function getDispatchRouter() {
        foreach($this->dispatchRouters as $r) {
            if (!$r->deferringToSource()) {
                return $r;
            }
        }
        return FALSE;
