...
class Plugin_facebook_Helper_userauth_oauth extends Helper_userauth {
	...
	function onLogin() {
		...
		if ($token = @$_SESSION['facebook_access_token']) {
			...
			if ($USER->display_name==$USER->facebook_full_name) {
				$USER->display_name = $USER->facebook_full_name = $this->formatName($me);
				$doSave = TRUE;
			}
		}
	...
	protected function registrationVars($me) {
		$vars = array();
		$vars['display_name'] = $vars['facebook_full_name'] = $this->formatName($me);
